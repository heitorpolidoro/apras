...
#:
#: APRAS-51 adds a **fourth** group of 25 below those three: the routes whose
#: mapped permission the map claimed and the code did not enforce. It is
#: imported from `tests/test_permission_alignment.py` rather than re-typed --
...
def test_the_permission_guarded_routes_are_the_apras43_seven_plus_billing():
    ...
    eighteen, plus APRAS-51's twenty-five. The lists are kept apart on purpose:
    the seven are a *conversion* and must never grow, while a genuinely new
    ...
    claim to be wrong. APRAS-51's group cannot be derived -- its routes have
    nothing in common but the defect -- so it is a literal, kept in
    `test_permission_alignment.py` where the structural walk that proves the
    form also lives.
    """
