...
def _migration_scoped_tables() -> tuple[str, ...]:
    """Read `_TENANT_SCOPED_TABLES` out of the single migration with `ast`.

    APRAS-58 collapsed the 39-revision history into one autogenerated
    revision, so the literal is no longer a frozen 2026-08 artefact needing a
    rename map: it names every directly-scoped table under its current
    spelling. `ast` rather than an import, because anything under `alembic/`
    shadows the real third-party library.
    """
...
def test_registry_matches_the_migrations_scoped_table_list():
    ...
    The registry picks a table up by *discovery* (any mapped class with a
    `tenant_id` column) and the migration spells the same set as a literal.
    Since APRAS-58 there is one migration, generated from the models, so the
    two sides are compared directly: no addendum set, and no rename map.
    """
