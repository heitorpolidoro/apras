...
BLOCKED_ROLES = [UserRole.RESIDENT, UserRole.PORTEIRO, UserRole.GUEST]
...
def admin(session: Session) -> User:
    return _make_user(session, UserRole.ADMINISTRATOR, "admin_rb@test.com", "11111111111")


@pytest.fixture
def director(session: Session) -> User:
    return _make_user(session, UserRole.DIRECTOR, "director_rb@test.com", "22222222222")


@pytest.fixture
def manager(session: Session) -> User:
    return _make_user(session, UserRole.MANAGER, "manager_rb@test.com", "33333333333")


@pytest.fixture
def other_manager(session: Session) -> User:
    return _make_user(session, UserRole.MANAGER, "manager2_rb@test.com", "77777777777")


@pytest.fixture
def blocked_users(session: Session) -> list[User]:
    return [
        _make_user(session, UserRole.RESIDENT, "resident_rb@test.com", "44444444444"),
        _make_user(session, UserRole.PORTEIRO, "porteiro_rb@test.com", "55555555555"),
        _make_user(session, UserRole.GUEST, "guest_rb@test.com", "66666666666"),
    ]
