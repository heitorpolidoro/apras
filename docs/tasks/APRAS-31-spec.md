...
New revision: `backend/alembic/versions/0022_add_space_reservation_tables.py`, `down_revision = "0021_add_feedback_table"`, as of this spec's original writing. **Post-implementation correction:** by the time of implementation, master had already advanced past `0021_add_feedback_table` — and then, mid-implementation, a concurrently-merged task (APRAS-32) claimed migration number `0022` for `0022_add_package_table.py`. The actual landed migration is `backend/alembic/versions/0023_add_space_reservation_tables.py`, `down_revision = "0022_add_package_table"`, confirmed via `alembic heads` showing a single linear head after rebasing onto master. This is exactly the scenario this section already warned about ("do not assume `0021_add_feedback_table` is still current without checking") — it just took two renumberings instead of one.
...
- Approving is **not** unconditional: two overlapping `PENDING` requests for the same `requires_approval` space are expected to coexist as normal, sequential competing bids — `create_reservation`'s conflict check only counts already-`CONFIRMED` rows as blocking, not other `PENDING` ones, so a second bid is never rejected at creation time just because an earlier bid for an overlapping window is still awaiting a decision. Because of this, the approve action **re-runs the double-booking check** (excluding the reservation being approved) against that space's current `CONFIRMED` reservations before flipping status. If a conflict is found — e.g. a different overlapping `PENDING` bid on the same space was already approved first — approving raises `SpaceReservationConflictError` (409) — the admin must reject this one or resolve the conflict manually; approving does not silently reject the other side. This re-check is a first-class, reachable part of the approval workflow via ordinary sequential requests, not a rare-race safeguard (see Non-Goals for the separate, still out-of-scope true-concurrency race).
...
- `SpaceReservationService.check_conflict(session, space_id, start_time, end_time, exclude_reservation_id=None) -> bool`: the mandatory double-booking check. Query:
  ...
  **Post-implementation correction:** the query above (`CONFIRMED`+`PENDING`) is `check_conflict`'s *default* when no `statuses` override is given, but every actual call site in this service passes an explicit `statuses=[ReservationStatus.CONFIRMED]` override — both `create_reservation` and `decide_reservation`'s approval re-check only treat already-`CONFIRMED` rows as blocking. This is deliberate, not an oversight: it's what allows multiple overlapping `PENDING` requests to coexist as competing bids on a `requires_approval` space until staff decides one (see "Approval / auto-confirm flow" below). If `create_reservation` conflict-checked against `PENDING` too, a second competing bid could never be filed, and the approval re-check's `SpaceReservationConflictError` path would be unreachable through any normal sequential request — only through the separate, narrower true-concurrency race that Non-Goals discusses.
- `SpaceReservationService.create_reservation(session, current_user, reservation_in) -> SpaceReservation`: loads the space (404 via `ReservableSpaceNotFoundError` if missing/inactive), runs `check_conflict` with `statuses=[ReservationStatus.CONFIRMED]`, raises `SpaceReservationConflictError` if it returns `True`, otherwise inserts with `status = PENDING if space.requires_approval else CONFIRMED`, `reserved_by_id = current_user.id`.
...
- No true row-level locking (`SELECT ... FOR UPDATE`) around the double-booking check — this app has no existing precedent for it, and introducing one is out of scope for this task. **Post-implementation correction:** `check_conflict` at creation time (and at approval time) only counts already-`CONFIRMED` reservations as blocking, not `PENDING` ones — this is deliberate, not the race window (see "Approval / auto-confirm flow" above: it lets multiple overlapping `PENDING` requests coexist as competing bids on a `requires_approval` space, which is what makes the approval re-check a reachable, normal-flow feature rather than dead code). The still-accepted, still out-of-scope race this bullet refers to is narrower: two `create_reservation` calls that are *truly concurrent* (both read the DB, both see no conflicting `CONFIRMED` row, and both commit) for the exact same slot on a space that does **not** require approval (where a reservation is `CONFIRMED` immediately) — or, for a `requires_approval` space, two truly-concurrent *approval* decisions on two different overlapping `PENDING` requests. The application-level check-then-insert/check-then-update has a narrow theoretical race window for that scenario; accepted as a known limitation for v1, consistent with how tightly this mirrors the draft's own framing of the check as "the core value proposition" rather than a fully lock-free-race-proof guarantee.
...
- **Backend** (`backend/tests/test_reservable_spaces.py`, `backend/tests/test_space_reservations.py` — new files, mirror `test_categories.py`'s and `test_occurrences.py`'s structure):
  ...
  - Double-booking: two reservations with overlapping windows on the same space where the first is (or becomes) `CONFIRMED` → the second `create_reservation` call gets `409 SpaceReservationConflictError` (this is the common case for a `requires_approval=False` space, where every successful create is immediately `CONFIRMED`). Adjacent-but-non-overlapping windows (`end_time` of one == `start_time` of the next) succeed (half-open interval, not inclusive). Overlapping windows on *different* spaces both succeed (conflict check is scoped to `space_id`). A `CANCELLED`/`REJECTED` reservation does not block a new overlapping one. On a `requires_approval=True` space, two overlapping `PENDING` requests are *not* blocked at creation time — see "Approval / auto-confirm flow" — so add a dedicated test confirming both sequential creates succeed as `PENDING` in that case, separately from the approve-time conflict test below.
...
4. Creating a reservation that overlaps an existing `CONFIRMED` reservation on the **same** space is rejected with `409`, enforced server-side (verified by a test that bypasses/ignores any client-side calendar check). Overlaps on different spaces, or against `CANCELLED`/`REJECTED` reservations, are allowed. Overlapping `PENDING` requests on the same `requires_approval` space are allowed to coexist (see item 5) — they are not double-bookings until one is approved.
